function doPost(e) {
  ...
  var recaptcha = params['g-recaptcha-response'];
  var time = Date();
  
  var recaptchaurl = 'https://www.google.com/recaptcha/api/siteverify';
  var recaptchadata = 'secret=6LcTxBwUAAAAAAd4F24fmsCfZ0wcGNG4Aa49l3fW&response=' + recaptcha;
  var recaptchaoption = {
    'method': 'post',
    'payload': data
  }
  var recaptchares = JSON.parse(UrlFetchApp(recaptchaurl, recaptchaoption));
  if (recaptchares.success) {
    ...
    return ContentService.createTextOutput('captcha failed');
  }
}
